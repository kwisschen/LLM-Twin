...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Whether to run the LinkedIn CSV loader pipeline.",
)
@click.option(
    "--linkedin-csv-config",
    default="linkedin_csv_etl_christopher_chen.yaml",
    help="Filename of the LinkedIn CSV ETL config file.",
)
@click.option(
    "--run-chatgpt-gemini-etl",
    is_flag=True,
    default=False,
    help="Whether to run the ChatGPT + Gemini ETL pipeline.",
)
@click.option(
    "--chatgpt-gemini-config",
    default="chatgpt_gemini_etl_christopher_chen.yaml",
    help="Filename of the ChatGPT/Gemini ETL config file.",
)
@click.option(
    "--export-settings",
    is_flag=True,
...
def main(
    ...
    run_linkedin_csv: bool = False,
    linkedin_csv_config: str = "linkedin_csv_etl_christopher_chen.yaml",
    run_chatgpt_gemini_etl: bool = False,
    chatgpt_gemini_config: str = "chatgpt_gemini_etl_christopher_chen.yaml",
    ...
    if run_etl:
        ...
        assert pipeline_args["config_path"].exists(), f"Config file not found: {pipeline_args['config_path']}"
        # --- NEW CODE TO LOAD THE YAML DATA ---
        with open(pipeline_args["config_path"], "r") as f:
            config_data = yaml.safe_load(f)
        
        run_args_etl = {
            "user_full_name": config_data.get("user_full_name"),
    ...
    if run_linkedin_csv:
        run_args_linkedin = {}
        pipeline_args["config_path"] = root_dir / "configs" / linkedin_csv_config
        assert pipeline_args["config_path"].exists(), f"Config file not found: {pipeline_args['config_path']}"
        with open(pipeline_args["config_path"], "r") as f:
            config_data = yaml.safe_load(f)
        run_args_linkedin = {
            "user_full_name": config_data["parameters"]["user_full_name"],
            "export_dir": config_data["parameters"]["export_dir"],
        }
        pipeline_args["run_name"] = f"linkedin_csv_etl_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
        linkedin_csv_etl.with_options(**pipeline_args)(**run_args_linkedin)

    if run_chatgpt_gemini_etl:
        run_args_cg = {}
        pipeline_args["config_path"] = root_dir / "configs" / chatgpt_gemini_config
        assert pipeline_args["config_path"].exists(), f"Config file not found: {pipeline_args['config_path']}"
        with open(pipeline_args["config_path"], "r") as f:
            config_data = yaml.safe_load(f)
        run_args_cg = {
            "user_full_name": config_data["parameters"]["user_full_name"],
            "chatgpt_export_path": config_data["parameters"]["chatgpt_export_path"],
            "gemini_export_path": config_data["parameters"]["gemini_export_path"],
        }
        pipeline_args["run_name"] = f"chatgpt_gemini_etl_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
        chatgpt_gemini_etl.with_options(**pipeline_args)(**run_args_cg)
