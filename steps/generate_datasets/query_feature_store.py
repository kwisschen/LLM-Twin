...
def fetch_all_data() -> dict[str, list[NoSQLBaseDocument]]:
    with ThreadPoolExecutor() as executor:
        future_to_query = {
            # Disabled: articles are from book authors (Paul Iusztin, Maxime Labonne), not Christopher's content.
            # executor.submit(
            #     __fetch_articles,
            # ): "articles",
            executor.submit(
...
def __fetch_conversations() -> list[CleanedDocument]:
    return __fetch(CleanedConversationDocument)
