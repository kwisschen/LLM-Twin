...
class LinkedInCrawler(BaseSeleniumCrawler):
    ...
    def login(self) -> None:
        ...
        logger.info("Waiting 60s for manual 2FA verification...")
        time.sleep(60)
        logger.info("2FA wait complete. Resuming crawl.")
    ...
    @staticmethod
    def test_selectors(profile_url: str = "https://www.linkedin.com/in/kwisschen/") -> None:
        """Open a visible Chrome session to inspect LinkedIn's current DOM structure.
        Does NOT scroll or extract — only navigates and dumps post container HTML."""

        options = webdriver.ChromeOptions()
        options.add_argument("--no-sandbox")
        # options.add_argument("--headless=new")  # Disabled for visual inspection
        ...
        logger.info("Browser open. Please log in manually (handle 2FA if needed).")
        logger.info("Waiting 90s for manual login...")
        time.sleep(90)
        ...
        # Step 3: Parse and inspect post containers
        soup = BeautifulSoup(driver.page_source, "html.parser")

        # Check the legacy selectors
        legacy_posts = soup.find_all(
            "div",
            class_="update-components-text relative update-components-update-v2__commentary",
        )
        logger.info(f"Legacy post selector matched: {len(legacy_posts)} elements")

        legacy_buttons = soup.find_all("button", class_="update-components-image__image-link")
        logger.info(f"Legacy image button selector matched: {len(legacy_buttons)} elements")

        # Dump raw HTML of any container with 'update-components' in its class
        all_update_divs = soup.find_all("div", class_=lambda c: c and "update-components" in c)
        logger.info(f"All 'update-components' divs found: {len(all_update_divs)}")
        for i, div in enumerate(all_update_divs[:5]):
            logger.info(f"\n--- update-components div [{i}] classes: {div.get('class')} ---")
            print(div.prettify()[:500])

        # Also check for any post-like containers
        feed_updates = soup.find_all("div", class_=lambda c: c and "feed-shared" in c)
        logger.info(f"'feed-shared' divs found: {len(feed_updates)}")
        for i, div in enumerate(feed_updates[:5]):
            logger.info(f"\n--- feed-shared div [{i}] classes: {div.get('class')} ---")
            print(div.prettify()[:500])

        logger.info("Inspection complete. Browser left open for manual review. Close it when done.")
