...
class ConversationEmbeddingHandler(EmbeddingDataHandler):
    def embed_batch(self, data_model: list[ConversationChunk]) -> list[EmbeddedConversationChunk]:
        # Embed only the prompt portion (stored in metadata) instead of full content
        embedding_model_input = [dm.metadata.get("prompt", dm.content) for dm in data_model]
        embeddings = embedding_model(embedding_model_input, to_list=True)

        return [
            self.map_model(dm, cast(list[float], emb))
            for dm, emb in zip(data_model, embeddings, strict=False)
        ]

    def map_model(self, data_model: ConversationChunk, embedding: list[float]) -> EmbeddedConversationChunk:
        return EmbeddedConversationChunk(
            id=data_model.id,
            content=data_model.content,
            embedding=embedding,
            platform=data_model.platform,
