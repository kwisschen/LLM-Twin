...
class ContextRetriever:
    ...
    def search(
        ...
        # Embed all queries on the main thread before parallel Qdrant search.
        # The SentenceTransformer singleton is not thread-safe — concurrent encode()
        # calls cause sporadic failures and empty embeddings.
        embedded_queries = [EmbeddingDispatcher.dispatch(q) for q in n_generated_queries]

        with concurrent.futures.ThreadPoolExecutor() as executor:
            search_tasks = [
                executor.submit(self._search_with_embedded_query, eq, k) for eq in embedded_queries
            ]
    ...
    def _search_with_embedded_query(self, embedded_query: EmbeddedQuery, k: int = 3) -> list[EmbeddedChunk]:
        """Run Qdrant vector search with a pre-computed embedding. Thread-safe."""
        assert k >= 3, "k should be >= 3"

        def _search_data_category(
            data_category_odm: type[EmbeddedChunk], eq: EmbeddedQuery
        ) -> list[EmbeddedChunk]:
            if eq.author_id:
                query_filter = Filter(
                    must=[
