    ...
    def get_prompts(cls, documents: list[CleanedDocument]) -> dict[DataCategory, list[GenerateDatasetSamplesPrompt]]:
        ...
        for category, category_documents in grouped_cleaned_documents.items():
            if category == DataCategory.CONVERSATIONS:
                # Conversations are already atomic Q&A pairs — skip sentence splitting,
                # just filter out short documents.
                category_documents = [doc for doc in category_documents if len(doc.content) >= 200]
            else:
                category_documents = generation_utils.extract_substrings(category_documents)
